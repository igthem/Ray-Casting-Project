    ...
    sf::Vector3f cam, sf::Vector3f** vecs)
{
    ...
    for (int j = 0; j < screen_high; j++)
    {
        for (int i = 0; i < screen_width * 4; i += 4)
        {
            ...
            // Выбор первого элемента, предпологая что он не наибольший.
            float max_t = v_dt[0];
            ...
            {
                // Предпологаем, что наибольший элемент являится одновременно наименьшим
                // Так мы избовлямсе от ошибки 
                float min_t = max_t;
